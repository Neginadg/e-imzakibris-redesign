...
// Hosts per the PayPoint TRNC API spec v1.5. The spec's own host table lists
// the production host as "https://paypointcyprus.com/ecom" — /ecom is the
// app's base path for everything (REST API included), not just the hosted
// gateway page. Confirmed 2026-07: a live reg call against
// "https://paypointcyprus.com/api/acquiring/reg" (i.e. without /ecom) returned
// HTTP 405, while PayPoint separately confirmed "https://paypointcyprus.com/ecom"
// as the checkout redirect target — so both REST calls and the gateway page
// share the same /ecom base. Override via PAYPOINT_API_BASE_URL /
// PAYPOINT_ECOM_BASE_URL env vars without a code change if this ever needs
// correcting again.
function getPaypointHosts(config) {
  if (config.paypointEnv === 'production') {
    return {
      ...
      ecomBase: config.paypointEcomBaseUrl || 'https://paypointcyprus.com/ecom'
    };
  }
