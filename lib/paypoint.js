...
// Hosts per the PayPoint TRNC API spec v1.5. The spec's own host table for
// the REST API lists the production host as "https://paypointcyprus.com/ecom"
// (already including /ecom) while the gateway-page POST section lists the
// same host *plus* a separate "/ecom" Url suffix — that combination looks
// like a documentation copy-paste error rather than an intentional
// "/ecom/ecom" path. We use the more sensible reading below; if PayPoint
// confirms otherwise, override via PAYPOINT_API_BASE_URL / PAYPOINT_ECOM_BASE_URL
// env vars without needing a code change.
function getPaypointHosts(config) {
  if (config.paypointEnv === 'production') {
    return {
      apiBase: config.paypointApiBaseUrl || 'https://paypointcyprus.com',
