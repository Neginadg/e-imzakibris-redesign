(function () {
  ...
  const TURNSTILE_SITE_KEY = String(
    (window.EIMZA_TURNSTILE_CONFIG && window.EIMZA_TURNSTILE_CONFIG.siteKey) ||
      "",
  ).trim();
  ...
  function enforceUppercaseNameInput(input) {
    ...
    input.addEventListener("input", function () {
      ...
      const newCaret = Math.max(
        0,
        (caret || after.length) - (before.length - after.length),
      );
  ...
  (function initImageHints() {
    if (document.readyState === "loading") {
      document.addEventListener("DOMContentLoaded", applyImageLoadingHints, {
        once: true,
      });
    } else {
  ...
  async function insertSupabaseRow(tableName, payload) {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        errorText || `Supabase insert failed: ${response.status}`,
      );
    }
  ...
  async function insertContactMessageDirect(payload) {
    ...
    if (!response.ok) {
      throw new Error(
        responseText || `Supabase insert failed with status ${response.status}`,
      );
    }
  ...
  const EN_TRANSLATIONS = {
    "e-imza KIBRIS - K.K.T.C.'nin güvenilir elektronik imza hizmet saglayicisi. Güvenli, hizli ve yasal geçerlilige sahip elektronik imza çözümleri.":
      "e-imza KIBRIS - The trusted electronic signature service provider in the TRNC. Secure, fast and legally valid e-signature solutions.",
    ...
    "İmzalama ve doğrulama masaüstü yazılımı":
      "Desktop signing and verification software",
    ...
    "Sertifika zinciri ve geçerlilik bilgileri":
      "Certificate chain and validity information",
    "Sertifika İptal Listeleri": "Certificate Revocation Lists",
    "İptal edilen sertifikalar ve CRL bilgileri":
      "Revoked certificates and CRL information",
    Sözleşmeler: "Agreements",
    "Bireysel, kurumsal ve kayıt makamı sözleşmeleri":
      "Individual, corporate and registration authority agreements",
    Mevzuat: "Legislation",
    "Yasa, tüzük ve düzenleme belgeleri":
      "Laws, regulations and legislative documents",
    ...
    "NES uygulama ve zaman damgası esasları":
      "NES application and timestamp guidelines",
    "İlgili Kurum Linkleri": "Relevant Institution Links",
    "Resmi ve kurumsal dış bağlantılar":
      "Official and corporate external links",
    ...
    "93/2007 Sayılı Elektronik İmza Yasası":
      "Electronic Signature Law No. 93/2007",
    ...
    "Online başvuru formunu doldurarak birkaç adımda e-imzanizi edinin.":
      "Fill out the online application form and get your e-signature in a few steps.",
    ...
    "Belge adına göre seçim yaparak ilgili dosyayı yeni sekmede açabilirsiniz.":
      "Select a document name to open the relevant file in a new tab.",
    ...
    "e-İmza Uygulamasında Akıllı Çubuk Kullanımı":
      "Using Smart Token in e-Signature Application",
  ...
  function showSubmitResultPopup(type, message) {
    ...
    if (title) {
      title.textContent =
        type === "success" ? "Başvuru Başarılı" : "Başvuru Başarısız";
    }
  ...
  const EN_TRANSLATION_LOOKUP = Object.fromEntries(
    Object.entries(EN_TRANSLATIONS).map(([source, target]) => [
      normalizeTranslationText(source),
      target,
    ]),
  );
  ...
  function translateCurrentPage(lang) {
    ...
    textNodes.forEach((node) => {
      ...
      const replacement =
        lang === "en" ? translateByLanguage(original, "en") : original;
    ...
    ["title", "aria-label", "placeholder", "alt"].forEach((attribute) => {
      document.querySelectorAll(`[${attribute}]`).forEach((el) => {
        ...
        const replacement =
          lang === "en" ? translateByLanguage(original, "en") : original;
        if (replacement !== el.getAttribute(attribute))
    ...
    const titleReplacement =
      lang === "en" ? translateByLanguage(originalTitle, "en") : originalTitle;
    ...
    if (metaDescription) {
      ...
      const replacement =
        lang === "en" ? translateByLanguage(originalDesc, "en") : originalDesc;
      if (replacement !== metaDescription.getAttribute("content"))
        metaDescription.setAttribute("content", replacement);
    }
  }
  ...
  function renderUploadedCertificates() {
    ...
    const rowsHtml = records
      .map((item) => {
        const fileName =
          item.name || `${item.documentName || "certificate"}.crt`;
  ...
  function applyPrices(prices) {
    ...
    document.querySelectorAll("[data-price-display]").forEach(function (el) {
      ...
      if (key && prices[key] != null)
        el.textContent = fmt.format(prices[key]) + " ₺";
    });
    ...
    const renewalInputs = document.querySelectorAll(
      'input[name="renewalTerm"]',
    );
    ...
    const molohiyaInputs = document.querySelectorAll(
      'input[name="molohiyaLicense"]',
    );
    const molohiyaOptions = document.querySelectorAll(
      'input[name="molohiyaLicense"]',
    );
    if (molohiyaInputs.length && molohiyaOptions.length) {
      molohiyaInputs.forEach(function (input, index) {
        ...
        const title = optionLabel
          ? (optionLabel.querySelector("strong") || {}).textContent ||
            "MOlOhiya Lisans"
          : "MOlOhiya Lisans";
    ...
    mobileNav.addEventListener("click", (event) => {
      ...
      if (toggleBtn && mobileNav.contains(toggleBtn)) {
        ...
        const groups = Array.from(
          mobileNav.querySelectorAll(".mobile-nav__group"),
        );
  ...
  function ensureSupportDropdownOptions() {
    const navItems = Array.from(
      document.querySelectorAll(
        ".navbar__nav .nav-list > .nav-item.has-dropdown",
      ),
    );
    ...
    const links = Array.from(
      dropdown.querySelectorAll(":scope > .dropdown__item"),
    );
    const sourceHref =
      (
        links.find((link) =>
          (link.getAttribute("href") || "")
            .toLowerCase()
            .includes("onlineapplication.html"),
        ) || links[0]
      )?.getAttribute("href") || "support/onlineapplication.html";

    const helpdeskHref = sourceHref.replace(
      /onlineapplication\.html$/i,
      "helpdesk.html",
    );
  ...
  function ensureAboutDropdownOptions() {
    ...
    const aboutItem = navItems.find((item) => {
      ...
      const text = (trigger?.textContent || "").toUpperCase();
      return (
        trigger && (text.includes("HAKKIMIZDA") || text.includes("ABOUT US"))
      );
    });
    if (!aboutItem) return;
    ...
    const hasCertLink = links.some((link) => {
      ...
      const text = (link.textContent || "").toLowerCase();
      return (
        href.includes("certdocuments.html") ||
        text.includes("belge ve sertifikalar") ||
        text.includes("documents and certificates")
      );
    });
    ...
    const hasAnnouncementsLink = links.some((link) => {
      ...
      const text = (link.textContent || "").toLowerCase();
      return (
        href.includes("announcements.html") ||
        text.includes("duyurular") ||
    ...
    const companyLink = links.find((link) => {
      ...
      const text = (link.textContent || "").toLowerCase();
      return (
        href.includes("about/index.html") ||
        href === "index.html" ||
        text.includes("şirket hakkında") ||
        text.includes("sirket hakkinda") ||
    ...
    const announcementsHref = sourceHref.replace(
      /index\.html$/i,
      "announcements.html",
    );
    const newsLink = links.find((link) =>
      (link.getAttribute("href") || "").toLowerCase().includes("news.html"),
    );
    ...
    if (!hasAnnouncementsLink) {
      ...
      const latestNewsLink = dropdown.querySelector(
        ':scope > .dropdown__item[href*="news.html"]',
      );
  ...
  function setupMobileNavFromDesktop() {
    ...
    const desktopItems = Array.from(
      document.querySelectorAll(".navbar__nav .nav-list > .nav-item"),
    );
    ...
    desktopItems.forEach((item, index) => {
      ...
      const dropdownLinks = dropdown
        ? Array.from(dropdown.querySelectorAll(".dropdown__item"))
        : [];
      ...
      if (dropdownLinks.length) {
        ...
        dropdownLinks.forEach((sourceLink) => {
          ...
          const itemLabel = (
            strong ? strong.textContent : sourceLink.textContent || ""
          ).trim();
          if (!itemLabel) return;
      ...
      try {
        ...
        try {
          ...
          setFormMessage(
            contactMessage,
            "success",
            "Mesajınız kaydedildi. Ekibimiz en kısa sürede size dönecek.",
          );
        ...
        setFormMessage(
          contactMessage,
          "danger",
          error.message || "Mesaj kaydedilemedi. Lütfen tekrar deneyin.",
        );
      } finally {
        if (submitButton) {
  ...
  if (builderForm) {
    ...
    const planInputs = Array.from(
      document.querySelectorAll('input[name="certificatePlan"]'),
    );
    ...
    const finalizeSendBtn = document.getElementById(
      "finalize-application-send",
    );
    ...
    const backToDetailsFromGateBtn = document.getElementById(
      "back-to-details-from-gate",
    );
    const confirmPaymentMethodBtn = document.getElementById(
    ...
    const applicationSubmitMessage = document.getElementById(
      "application-submit-message",
    );
    const detailsForm = document.getElementById("application-details-form");
    const birthDateInput = detailsForm
      ? detailsForm.querySelector('input[name="birthDate"]')
      : null;
    enforceUppercaseNameInput(
      detailsForm ? detailsForm.querySelector('input[name="fullName"]') : null,
    );
    ...
    const summaryCertificatePrice = document.getElementById(
      "summary-certificate-price",
    );
    ...
    const validateBirthDate = () => {
      ...
      if (Number.isNaN(selectedDate.getTime())) {
        birthDateInput.setCustomValidity(
          "Lutfen gecerli bir dogum tarihi giriniz.",
        );
        return false;
      }
    ...
    const setActiveStep = (step) => {
      stepIndicators.forEach((indicator) => {
        indicator.classList.toggle(
          "is-active",
          Number(indicator.dataset.stepIndicator) === step,
        );
      });
    };

    const selectedPlan = () =>
      planInputs.find((input) => input.checked) || planInputs[0];
    ...
    const updateSummary = () => {
      ...
      const setupPrice =
        setupInput && setupInput.checked ? Number(setupInput.value) : 0;
      ...
      if (finalPaymentAmount)
        finalPaymentAmount.value = currencyFormatter.format(total);
    ...
    const applyPlanFromQuery = () => {
      ...
      const matchedPlan = planInputs.find(
        (input) => input.dataset.priceInput === planFromQuery,
      );
      if (matchedPlan) {
    ...
    if (detailsForm) {
      detailsForm.addEventListener("submit", (event) => {
        ...
        const professionalRegistryNo = String(
          formData.get("professionalRegistryNo") || "-",
        );
        const publicDirectoryConsent = formData.get("publicDirectoryConsent")
          ? "Evet"
          : "Hayir";
        const company = String(formData.get("company") || "-");
        const jobTitle = String(formData.get("jobTitle") || "-");
        const email = String(formData.get("email") || "");
        const showEmailOnCertificate = formData.get("showEmailOnCertificate")
          ? "Evet"
          : "Hayir";
        ...
        const invoiceSameAsContact = formData.get("invoiceSameAsContact")
          ? "Evet"
          : "Hayir";
        ...
        const invoiceType = String(
          formData.get("invoiceType") || "Dijital Fatura",
        );
    ...
    const paymentTeslimattaNotice = document.getElementById(
      "payment-teslimatta-notice",
    );
    ...
    paymentMethodInputs.forEach((input) => {
      input.addEventListener("change", () => {
        if (paymentTeslimattaNotice) {
          paymentTeslimattaNotice.style.display =
            input.value === "Teslimatta Ödeme" ? "block" : "none";
        }
      });
    });
        ...
        if (!selectedMethod) {
          if (paymentMethodNote) {
            paymentMethodNote.innerHTML =
              '<i class="fa-solid fa-triangle-exclamation"></i><p>Lütfen devam etmek için bir ödeme yöntemi seçiniz.</p>';
          }
          return;
        }

        // Kredi Kartı: submit the pending application, then redirect to PayPoint's hosted gateway page
        if (selectedMethod.value === "Kredi Kartı") {
          if (!pendingApplicationSubmission || !pendingApplicationMail) {
            if (paymentMethodNote) {
              paymentMethodNote.innerHTML =
                '<i class="fa-solid fa-triangle-exclamation"></i><p>Başvuru verileri eksik. Lütfen formu kontrol edin.</p>';
            }
            return;
          }
          ...
          const payload = {
            ...
            amount: Math.round(pendingApplicationMail.totalAmount * 100),
            payload: {
          ...
          try {
            ...
            confirmPaymentMethodBtn.innerHTML =
              '<i class="fa-solid fa-spinner fa-spin"></i> Ödeme sayfasına yönlendiriliyor...';

            const response = await postBackendForm(
              "/api/application-submit",
              payload,
            );
            if (!response || !response.creditCard) {
              throw new Error("Ödeme başlatılamadı.");
            }

            window.EIMZA_PAYPOINT.redirectToCheckout(response.creditCard);
            return;
          } catch (error) {
            if (paymentMethodNote) {
              paymentMethodNote.innerHTML =
      ...
      finalizeSendBtn.addEventListener("click", async () => {
        ...
        try {
          ...
          setFormMessage(
            applicationSubmitMessage,
            "danger",
            error.message || "Başvuru kaydedilemedi. Lütfen tekrar deneyin.",
          );
          setActiveStep(3);
          scrollToSection(submitSection);
        } finally {
  ...
  if (tsBuilderForm) {
    ...
    const tsPlanInputs = Array.from(
      document.querySelectorAll('input[name="timestampPlan"]'),
    );
    ...
    const tsPaymentMethodGate = document.getElementById(
      "ts-payment-method-gate",
    );
    const tsPaymentFinalPanel = document.getElementById(
      "ts-payment-final-panel",
    );
    const tsPaymentMethodNote = document.getElementById(
      "ts-payment-method-note",
    );
    const tsBackToDetailsFromGateBtn = document.getElementById(
      "ts-back-to-details-from-gate",
    );
    const tsConfirmPaymentMethodBtn = document.getElementById(
      "ts-confirm-payment-method",
    );
    ...
    const tsStepIndicators = Array.from(
      document.querySelectorAll("[data-ts-step-indicator]"),
    );

    const tsSummaryPackagePrice = document.getElementById(
      "ts-summary-package-price",
    );
    const tsSummaryTotalPrice = document.getElementById(
      "ts-summary-total-price",
    );
    ...
    const setActiveTsStep = (step) => {
      tsStepIndicators.forEach((indicator) => {
        indicator.classList.toggle(
          "is-active",
          Number(indicator.dataset.tsStepIndicator) === step,
        );
      });
    };

    const selectedTsPlan = () =>
      tsPlanInputs.find((input) => input.checked) || tsPlanInputs[0];

    const pad2 = (value) => String(value).padStart(2, "0");

    const populateSelectRange = (
      selectElement,
      start,
      end,
    ...
    const updateTsSummary = () => {
      ...
      if (tsFinalPaymentAmount)
        tsFinalPaymentAmount.value = currencyFormatter.format(total);
    ...
    const tsApplicationTypeInputs = Array.from(
      tsDetailsForm?.querySelectorAll('input[name="applicationType"]') || [],
    );
    ...
    const hasTsApplicationType = () =>
      tsApplicationTypeInputs.some((input) => input.checked);
    const getTsApplicationType = () =>
      tsApplicationTypeInputs.find((input) => input.checked)?.value || "";
    ...
    const updateTsFormSections = () => {
      ...
      if (tsCorporateInfo) {
        ...
        const corporateFields =
          tsCorporateInfo.querySelectorAll("input, select");
    ...
    const updateTsApplicationTypeValidity = () => {
      const validityMessage = hasTsApplicationType()
        ? ""
        : "Lutfen basvuru tipini seciniz.";
    ...
    if (tsDetailsForm) {
      tsDetailsForm.addEventListener("submit", (event) => {
        ...
        const applicationType = normalizeTsApplicationType(
          getTsApplicationType(),
        );
    ...
    const tsPaymentMethodInputs = Array.from(
      document.querySelectorAll('input[name="tsPaymentMethod"]'),
    );

    if (tsConfirmPaymentMethodBtn) {
      tsConfirmPaymentMethodBtn.addEventListener("click", async () => {
        const selectedTsMethod = document.querySelector(
          'input[name="tsPaymentMethod"]:checked',
        );

        if (selectedTsMethod && selectedTsMethod.value === "Kredi Kartı") {
          if (!pendingTsSubmission || !pendingTsMail) {
            if (tsPaymentMethodNote) {
              tsPaymentMethodNote.innerHTML =
                '<i class="fa-solid fa-triangle-exclamation"></i><p>Başvuru verileri eksik. Lütfen formu kontrol edin.</p>';
            }
            return;
          }
          ...
          const payload = {
            ...pendingTsSubmission,
            payment_method: "Kredi Kartı",
            amount: Math.round(pendingTsMail.totalAmount * 100),
            payload: {
          ...
          try {
            ...
            tsConfirmPaymentMethodBtn.innerHTML =
              '<i class="fa-solid fa-spinner fa-spin"></i> Ödeme sayfasına yönlendiriliyor...';
      ...
      tsFinalizeSendBtn.addEventListener("click", async () => {
        ...
        try {
          const selectedPaymentMethod =
            document.querySelector('input[name="tsPaymentMethod"]:checked')
              ?.value || "Havale/EFT";
  ...
  if (renewalForm) {
    ...
    const renewalPaymentFinal = document.getElementById(
      "renewal-payment-final",
    );
    const renewalPaymentNote = document.getElementById("renewal-payment-note");
    const backToRenewalFormBtn = document.getElementById(
      "back-to-renewal-form",
    );
    const confirmRenewalPaymentMethodBtn = document.getElementById(
      "confirm-renewal-payment-method",
    );
    const backToRenewalGateBtn = document.getElementById(
      "back-to-renewal-gate",
    );
    const renewalFinalizeSendBtn = document.getElementById(
      "renewal-finalize-send",
    );
    const renewalFinalTotalPrice = document.getElementById(
      "renewal-final-total-price",
    );
    const renewalFinalPaymentAmount = document.getElementById(
      "renewal-final-payment-amount",
    );
    const renewalFinalSenderFirst = document.getElementById(
      "renewal-final-sender-first",
    );
    const renewalFinalSenderLast = document.getElementById(
      "renewal-final-sender-last",
    );
    const summaryTerm = document.getElementById("renewal-summary-term");
    const summaryTermPrice = document.getElementById(
      "renewal-summary-term-price",
    );
    const summaryLicense = document.getElementById("renewal-summary-license");
    const summaryLicensePrice = document.getElementById(
      "renewal-summary-license-price",
    );
    const summaryTotalPrice = document.getElementById(
      "renewal-summary-total-price",
    );
    ...
    const renewalTermInputs = renewalForm.querySelectorAll(
      'input[name="renewalTerm"]',
    );
    const molohiyaLicenseInputs = renewalForm.querySelectorAll(
      'input[name="molohiyaLicense"]',
    );
    let pendingRenewalMail = null;
    let pendingRenewalSubmission = null;
    let pendingRenewalTotalText = "-";
    let pendingRenewalTotalAmount = null;
    ...
    const getOptionTitle = (input) => {
      const titleNode = input
        ?.closest(".renewal-package-option")
        ?.querySelector(".renewal-package-option__meta strong");
      return titleNode ? titleNode.textContent.trim() : "Seçim yapılmadı";
    };

    const updateRenewalSummary = () => {
      const selectedTerm = renewalForm.querySelector(
        'input[name="renewalTerm"]:checked',
      );
      const selectedLicense = renewalForm.querySelector(
        'input[name="molohiyaLicense"]:checked',
      );

      const termTitle = selectedTerm
        ? getOptionTitle(selectedTerm)
        : "Seçim yapılmadı";
      const licenseTitle = selectedLicense
        ? getOptionTitle(selectedLicense)
        : "Seçilmedi";

      const termPrice = selectedTerm
        ? extractFinalPrice(selectedTerm.value)
        : null;
      const licensePrice = selectedLicense
        ? extractFinalPrice(selectedLicense.value)
        : null;
      ...
      if (summaryTermPrice)
        summaryTermPrice.textContent = formatPrice(termPrice);
      if (summaryLicensePrice)
        summaryLicensePrice.textContent = selectedLicense
          ? formatPrice(licensePrice)
          : "-";
      if (summaryTotalPrice)
        summaryTotalPrice.textContent =
          selectedTerm && hasTermPrice ? formatPrice(total) : "-";

      pendingRenewalTotalText =
        selectedTerm && hasTermPrice ? formatPrice(total) : "-";
      pendingRenewalTotalAmount = selectedTerm && hasTermPrice ? total : null;
      if (renewalFinalTotalPrice)
        renewalFinalTotalPrice.textContent = pendingRenewalTotalText;
      if (renewalFinalPaymentAmount)
        renewalFinalPaymentAmount.value =
          pendingRenewalTotalText === "-"
            ? "-"
            : pendingRenewalTotalText.replace(" ₺", "");
    };
    ...
    renewalTermInputs.forEach((input) =>
      input.addEventListener("change", updateRenewalSummary),
    );
    ...
    const renewalTeslimattaNotice = document.getElementById(
      "renewal-teslimatta-notice",
    );
    const renewalPaymentMethodInputs = Array.from(
      document.querySelectorAll('input[name="renewalPaymentMethod"]'),
    );

    renewalPaymentMethodInputs.forEach((input) => {
      input.addEventListener("change", () => {
        if (renewalTeslimattaNotice) {
          renewalTeslimattaNotice.style.display =
            input.value === "Teslimatta Ödeme" ? "block" : "none";
        }
      });
    });
    ...
    renewalForm.addEventListener("submit", (event) => {
      ...
      pendingRenewalMail = {
        subject: `Yenileme Basvurusu - ${renewalTerm}`,
        totalText: pendingRenewalTotalText,
        totalAmount: pendingRenewalTotalAmount,
      };
      pendingRenewalSubmission = submissionPayload;

      setRenewalPaymentGateView();
      scrollToSection(renewalPaymentGate);
    });

    if (confirmRenewalPaymentMethodBtn) {
      confirmRenewalPaymentMethodBtn.addEventListener("click", async () => {
        const selectedRenewalMethod = document.querySelector(
          'input[name="renewalPaymentMethod"]:checked',
        );
        if (!selectedRenewalMethod) {
          if (renewalPaymentNote) {
            renewalPaymentNote.innerHTML =
              '<p><i class="fa-solid fa-triangle-exclamation"></i> Lütfen devam etmek için bir ödeme yöntemi seçiniz.</p>';
          }
          return;
        }

        if (!pendingRenewalSubmission || !pendingRenewalMail) {
          if (renewalPaymentNote) {
            renewalPaymentNote.innerHTML =
              '<p><i class="fa-solid fa-triangle-exclamation"></i> Başvuru verileri eksik. Lütfen formu kontrol edin.</p>';
          }
          return;
        }

        if (selectedRenewalMethod.value === "Kredi Kartı") {
          if (
            !Number.isFinite(pendingRenewalMail.totalAmount) ||
            pendingRenewalMail.totalAmount <= 0
          ) {
            if (renewalPaymentNote) {
              renewalPaymentNote.innerHTML =
                '<p><i class="fa-solid fa-triangle-exclamation"></i> Lütfen önce bir yenileme paketi seçiniz.</p>';
            }
            return;
          }
          ...
          const payload = {
            ...
            payment_method: "Kredi Kartı",
            amount: Math.round(pendingRenewalMail.totalAmount * 100),
            payload: {
          ...
          try {
            ...
            confirmRenewalPaymentMethodBtn.innerHTML =
              '<i class="fa-solid fa-spinner fa-spin"></i> Ödeme sayfasına yönlendiriliyor...';
        ...
        if (selectedRenewalMethod.value === "Teslimatta Ödeme") {
          const payload = {
            ...pendingRenewalSubmission,
            payment_method: "Teslimatta Ödeme",
            payload: {
          ...
          try {
            ...
            if (renewalMessage) {
              setFormMessage(
                ...
                "danger",
                error.message ||
                  "Yenileme başvurusu kaydedilemedi. Lütfen tekrar deneyin.",
              );
            }
          } finally {
            confirmRenewalPaymentMethodBtn.disabled = false;
    ...
    if (renewalFinalizeSendBtn) {
      renewalFinalizeSendBtn.addEventListener("click", async () => {
        ...
        renewalFinalizeSendBtn.innerHTML =
          '<i class="fa-solid fa-spinner fa-spin"></i> Kaydediliyor...';
        ...
        try {
          const payload = {
            ...
            payload: {
              ...
              payment: {
                ...
                paymentAmount: pendingRenewalMail.totalText,
                paymentMethod: "Havale/EFT",
              },
            },
          };
          ...
          renewalFinalizeSendBtn.innerHTML = "Yenileme Talebini Gönder";
        }
      });
    }
  }
  ...
  const molohiyaForm = document.getElementById(
  ...
  if (molohiyaForm) {
    ...
    const molohiyaPaymentGate = document.getElementById(
      "molohiya-payment-gate",
    );
    const molohiyaPaymentFinal = document.getElementById(
      "molohiya-payment-final",
    );
    const molohiyaPaymentNote = document.getElementById(
      "molohiya-payment-note",
    );
    const molohiyaBackToDetails = document.getElementById(
      "molohiya-back-to-details",
    );
    const molohiyaConfirmPayment = document.getElementById(
      "molohiya-confirm-payment",
    );
    const molohiyaBackToGate = document.getElementById("molohiya-back-to-gate");
    const molohiyaFinalizeSend = document.getElementById(
      "molohiya-finalize-send",
    );
    const molohiyaFinalTotalPrice = document.getElementById(
      "molohiya-final-total-price",
    );
    const molohiyaFinalPaymentAmount = document.getElementById(
      "molohiya-final-payment-amount",
    );
    const molohiyaFinalSenderFirst = document.getElementById(
      "molohiya-final-sender-first",
    );
    const molohiyaFinalSenderLast = document.getElementById(
      "molohiya-final-sender-last",
    );
    ...
    const updateMolohiyaSubmitLabel = () => {
      ...
      if (!submitBtn) return;
      const termInput = molohiyaForm.querySelector(
        'input[name="renewalTerm"]:checked',
      );
      const price = termInput ? Number(termInput.dataset.price || 0) : 0;
      submitBtn.textContent =
        price === 0 ? "Başvuruyu Gönder" : "Satın Alma Talebini Gönder";
    };

    molohiyaForm
      .querySelectorAll('input[name="renewalTerm"]')
      .forEach((input) => {
    ...
    if (molohiyaConfirmPayment) {
      molohiyaConfirmPayment.addEventListener("click", async () => {
        const selectedMolohiyaMethod = document.querySelector(
          'input[name="molohiyaPaymentMethod"]:checked',
        );

        if (
          selectedMolohiyaMethod &&
          selectedMolohiyaMethod.value === "Kredi Kartı"
        ) {
          if (!pendingMolohiyaSubmission || !pendingMolohiyaMail) {
            if (molohiyaPaymentNote) {
              molohiyaPaymentNote.innerHTML =
                '<p><i class="fa-solid fa-triangle-exclamation"></i> Başvuru verileri eksik. Lütfen formu kontrol edin.</p>';
            }
            return;
          }
          ...
          const payload = {
            ...
            payment_method: "Kredi Kartı",
            amount: Math.round(pendingMolohiyaMail.totalAmount * 100),
            payload: {
          ...
          try {
            molohiyaConfirmPayment.disabled = true;
            molohiyaConfirmPayment.innerHTML =
              '<i class="fa-solid fa-spinner fa-spin"></i> Ödeme sayfasına yönlendiriliyor...';
            ...
            molohiyaConfirmPayment.disabled = false;
            molohiyaConfirmPayment.innerHTML = "İşlemi Onaylıyorum";
          }
    ...
    molohiyaForm.addEventListener("submit", async (event) => {
      ...
      if (molohiyaFinalTotalPrice)
        molohiyaFinalTotalPrice.textContent = molohiyaTotalText;
      if (molohiyaFinalPaymentAmount)
        molohiyaFinalPaymentAmount.value = currencyFormatter.format(total);
        ...
        try {
          ...
          if (molohiyaMessage) {
            setFormMessage(
              molohiyaMessage,
              "danger",
              error.message || "Başvuru gönderilemedi. Lütfen tekrar deneyin.",
            );
          }
        } finally {
    ...
    if (molohiyaFinalizeSend) {
      molohiyaFinalizeSend.addEventListener("click", async () => {
        ...
        molohiyaFinalizeSend.innerHTML =
          '<i class="fa-solid fa-spinner fa-spin"></i> Kaydediliyor...';
        ...
        try {
          ...
          if (molohiyaMessage) {
            setFormMessage(
              ...
              "danger",
              error.message ||
                "Satın alma talebi kaydedilemedi. Lütfen tekrar deneyin.",
            );
          }
        } finally {
